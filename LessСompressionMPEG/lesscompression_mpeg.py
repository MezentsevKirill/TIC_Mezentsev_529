...
def blockSearchBody(anchor, target, blockSize, searchArea=7):
    ...
    for y in range(0, int(hSegments*blockSize), blockSize):
        for x in range(0, int(wSegments*blockSize), blockSize):
            ...
            anchorSearchArea = getAnchorSearchArea(x, y, anchor, blockSize,
            searchArea)
            anchorBlock = getBestMatch(targetBlock, anchorSearchArea,
            blockSize)
...
def main(anchorFrame, targetFrame, saveOutput):
    blockSize = 16
    bitsAnchor = []
    bitsDiff = []
    bitsPredicted = []
