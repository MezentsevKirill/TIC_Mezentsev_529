...
def write_to_file(filepath, dc, ac, blocks_count, tables):
    ...
    for table_name in ['dc_y', 'ac_y', 'dc_c', 'ac_c']:
        # 16 біт для 'table_size'
        f.write(uint_to_binstr(len(tables[table_name]), 16))

        for key, value in tables[table_name].items():
            if table_name in {'dc_y', 'dc_c'}:
                # 4 біт для 'category'
                # 4 біт для 'code_length'
                # 'code_length' біт для 'huffman_code'
                f.write(uint_to_binstr(key, 4))
                f.write(uint_to_binstr(len(value), 4))
                f.write(value)
            else:
                # 4 біт для 'run_length'
                # 4 біт для 'size'
                # 8 біт для 'code_length'
...
class JPEGFileReader:
    ...
    def __init__(self, filepath):
        self.__file = open('./Results/' + filepath, 'r')
...
def decoder(input_file, output_file, original_image, image_type):
    ...
    image.save('./Results/' + output_file)

    size_jpeg = os.path.getsize('./Results/' + output_file)
    width, height = image.size
    ratio = os.path.getsize(original_image) / size_jpeg
    with open("results_jpeg.txt", "a", encoding='UTF-8') as file:
        print('Дані для ', image_type, ' зображення (таблиця квантування 2)', file=file)
...
def main():
    # 1-st quantization table
    # encode('8_3.bmp', '8_3_encoded.asf')
    # decoder('8_3_encoded.asf', 'JPEG_8_3_decoded.jpg', '8_3.bmp', 'слаботекстурного')
    #
    # encode('8_2.bmp', '8_2_encoded.asf')
    # decoder('8_2_encoded.asf', 'JPEG_8_2_decoded.jpg', '8_2.bmp', 'середньотекстурного')
    #
    # encode('8_1.bmp', '8_1_encoded.asf')
    # decoder('8_1_encoded.asf', 'JPEG_8_1_decoded.jpg', '8_1.bmp', 'сильнотекстурного')

    encode('8_3.bmp', '8_3_encoded_2.asf')
    decoder('8_3_encoded_2.asf', 'JPEG_8_3_decoded_2.jpg', '8_3.bmp', 'слаботекстурного')

    encode('8_2.bmp', '8_2_encoded_2.asf')
    decoder('8_2_encoded_2.asf', 'JPEG_8_2_decoded_2.jpg', '8_2.bmp', 'середньотекстурного')

    encode('8_1.bmp', '8_1_encoded_2.asf')
    decoder('8_1_encoded_2.asf', 'JPEG_8_1_decoded_2.jpg', '8_1.bmp', 'сильнотекстурного')
