...
def main():
    ...
    for sequence in original_sequences:
        ...
        with open("result_hamming.txt", "a", encoding="UTF-8") as file:
            print("/" * 20 + f"\nОригінальна послідовність: {source}", file=file)
            print(f"Оригінальна послідовність в бітах: {source_bin}", file=file)
            print(f"Розмір оригінальної послідовності: {str(len(source_bin))}", file=file)
            print(f"Довжина блоку кодування: {CHUNK_LENGTH}", file=file)
            print(f"Позиція контрольних біт: {CHECK_BITS}", file=file)
            print(f"Відносна надмірність коду: {len(CHECK_BITS)/CHUNK_LENGTH}", file=file)
            print("__________Кодування__________", file=file)
            print(f"Закодовані дані: {encoded}", file=file)
            print(f"Розмір закодованих даних: {len(encoded)}", file=file)
            print("_________Декодування_________", file=file)
            print(f"Декодовані дані: {decoded}", file=file)
            print(f"Розмір декодованих даних: {len(decoded) * 16}", file=file)
            print("______Внесення помилки_______", file=file)
            print(f"Закодовані дані з помилками: {encoded_with_error}", file=file)
            print(f"Кількість помилок: {len(diff_index_list)}", file=file)
            print(f"Індекси помилок: {diff_index_list}", file=file)
            print(f"Декодовані дані без виправлення помилки: {decoded_with_error}", file=file)
            print(f"Декодовані дані з виправленням помилки: {decoded_without_error}\n" + "/" * 20 + f"\n", file=file)
